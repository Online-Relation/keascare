// Regnskab-scraper til Synology NAS.
// Henter årsregnskab fra regnskab.virk.dk for alle bosteder med CVR.
//
// Krav: Node.js + npm-pakker installeret:
//   npm install axios @supabase/supabase-js ws
//
// Kørsel:
//   SUPABASE_URL=https://... SUPABASE_SERVICE_KEY=... node /volume1/scripts/regnskab-synology.js
//
// Eller med cronjob på Synology (Control Panel → Task Scheduler):
//   SUPABASE_URL=https://gclg... SUPABASE_SERVICE_KEY=sb_secret_... /volume1/@appstore/Node.js_v18/usr/local/bin/node /volume1/scripts/regnskab-synology.js
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
  ...
  console.error('Kør: SUPABASE_URL=https://... SUPABASE_SERVICE_KEY=... node regnskab-synology.js');
  process.exit(1);
}
...
async function kørRegnskabScraper() {
  ...
  if (!rækker?.length) { console.log('Ingen at behandle — alt er opdateret'); return 0; }
...
try {
  while (true) {
    ...
    const behandlet = await kørRegnskabScraper();

    if (behandlet === 0) break;

    // Tjek om der er flere tilbage
  ...
  await logKørsel(false, { error: err.message });
  process.exit(1);
}
