...
import { hentRegistreringer, opdaterNote, sletRegistrering } from '@/features/tidsregistrering/services/TidsregistreringService';
...
export function RegistreringerTabel() {
  ...
  const [noteVærdi, setNoteVærdi]           = useState('');
  ...
  async function gemNote(id: string) {
    await opdaterNote(id, noteVærdi);
    setRegistreringer((prev) => prev.map((r) => r.id === id ? { ...r, note: noteVærdi } : r));
    setRedigerer(null);
  }
                ...
                {rækker.map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="tr-note-celle">
                      {redigerer === r.id ? (
                        <div className="tr-note-edit">
                          <input
                            value={noteVærdi}
                            onChange={(e) => setNoteVærdi(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && gemNote(r.id)}
                            autoFocus
                            className="tr-note-input"
                          />
                          <button onClick={() => gemNote(r.id)} className="tr-ikon-btn"><Check size={13} /></button>
                          <button onClick={() => setRedigerer(null)} className="tr-ikon-btn"><X size={13} /></button>
                        </div>
                      ) : (
                        <span
                          className="tr-note-tekst"
                          onClick={() => { setRedigerer(r.id); setNoteVærdi(r.note ?? ''); }}
                        >
                          {r.note || <span className="tr-note-tom">Tilføj note…</span>}
                        </span>
                      )}
                    </td>
                    <td>
                      <button onClick={() => slet(r.id)} className="tr-ikon-btn tr-slet"><Trash2 size={13} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
