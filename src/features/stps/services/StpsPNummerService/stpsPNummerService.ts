...
export async function kørPNummerScraper(batch = 50): Promise<Record<string, unknown>> {
  ...
  for (let i = 0; i < rapporter.length; i++) {
    ...
    try {
      ...
      if (detaljer.pNummer) {
        await supabase.from('stps_rapporter').update({ p_nummer: detaljer.pNummer }).eq('id', id);
        fundet++;
      } else {
        ingenPNummer++;
      }
