...
export function KommunerPage({ kommuner }: Props) {
  ...
  return (
    <div className="dashboard-content">
      ...
      <div className="kommuner-tabel-wrapper">
        <table className="data-table">
          <thead>
            <tr>
              <th>Kommune</th>
              <th className="tal-kolonne">§107</th>
              <th className="tal-kolonne">§108</th>
          ...
          <tbody>
            {kommuner.map((k) => (
              <tr key={k.navn} className="kommuner-tabel-række">
                <td>
                  <Link
                    href={`/dashboard/kommuner/${encodeURIComponent(k.navn)}`}
                    className="kommuner-navn-link"
                  >
